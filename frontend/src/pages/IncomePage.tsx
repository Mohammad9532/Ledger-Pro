...
export default function IncomePage() {
  ...
  const fetchData = () => {
    setLoading(true);
    api.get(`/reports/income-summary?start_date=${startDate}&end_date=${endDate}&include_business=${includeBusiness}`).then(res => { setSummary(res.data); setLoading(false); });
    api.get('/income-categories').then(res => setCategories(res.data));
  };
  ...
  const handleDeleteCategory = async (id: number) => {
    ...
    try {
      ...
      fetchData();
    } catch (err: any) { alert(err.response?.data?.error || err.response?.data?.message || 'Failed to delete'); }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Category-wise income tracking and analysis</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Pie Chart */}
        <Card>
          <CardHeader><CardTitle className="text-lg">Income Breakdown</CardTitle></CardHeader>
          <CardContent>
            {chartData.length > 0 ? (
              <div className="h-72">
            ...
            ) : <p className="text-center text-muted-foreground py-8">No income in this period</p>}
          </CardContent>
        </Card>
              ...
              <tbody>
                {summary?.categories?.map((c: any, i: number) => (
                  <tr key={i} className="border-b hover:bg-accent/50 transition-colors">
                    ...
                    <td className="p-3 text-right font-medium text-emerald-500">{formatCurrency(c.total)}</td>
                  </tr>
                ))}
      ...
      <Card>
        <CardHeader><CardTitle className="text-lg">Income Categories</CardTitle></CardHeader>
        <CardContent>
